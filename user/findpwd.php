...
if (isset($_GET['act']) && $_GET['act'] == 'qrlogin') {
    if (isset($_SESSION['findpwd_qq']) && $qq = trim(daddslashes($_SESSION['findpwd_qq']))) {
        $res = $DB->get_row("select * from pre_user where qq=:qq limit 1", [':qq' => $qq]);
        unset($_SESSION['findpwd_qq']);
        if ($res['user']) {
            $session = md5($res['user'] . $res['pwd'] . $password_hash);
            $token = authcode("{$res['id']}\t{$session}", 'ENCODE', SYS_KEY);
            setcookie("user_token", $token, time() + 604800, '/');
            $DB->exec("update pre_user set lasttime='$date' where id='{$res["id"]}'");
