...
function create_link($params, $codepay_key, $host = "")
{
    ...
    $apiHost = $host ? $host : "http://codepay.kjkl8.com/api/orders/?";
...
$api_url = create_link($api_data, $conf['codepay_key'], 'http://codepay.kjkl8.com/api/orders/?'); //生成API请求地址
...
if ($arr && $arr['code'] == 1) {
    ...
    foreach ($arr['data'] as $row) {
        if ($row['status'] == 1 && $count < $limit) {
            ...
            if ($srow && $srow['status'] == 0) {
                $DB->exec("update `pre_pay` set `status` ='1',`endtime` ='$date' where `trade_no`=:trade_no", [':trade_no' => $out_trade_no]);
                processOrder($srow);
                $api_data['count'] += 1;
    ...
    if (!empty($api_data['trade_no'])) {
        $api_url = create_link($api_data, $conf['codepay_key'], 'http://codepay.kjkl8.com/api/up_orders/?'); //生成API请求地址
