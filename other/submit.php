...
<body>
	...
	$row = $DB->get_row("SELECT * FROM pre_pay WHERE trade_no=:trade_no limit 1", [':trade_no' => $orderid]);
	...
	$DB->exec("update `pre_pay` set `type` =:type where `trade_no`=:trade_no", [':type' => $type, ':trade_no' => $orderid]);
